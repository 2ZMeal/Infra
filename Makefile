...
help: ## 사용 가능한 명령어 목록
	...
	@echo ""
	@echo "  이미지 빌드"
	@echo "    build-platform   config-server, eureka-server 이미지 빌드"
	@echo ""
	@echo "  개발 환경 실행"
...
# 이미지 빌드
# ────────────────────────────────────────────
build-platform: ## config-server, eureka-server 이미지 재빌드
...
dev-platform: ## infra + platform 실행 (Config Server, Eureka — Gateway는 Milestone4 완료 후 추가)
	@docker compose --profile infra up -d
	@docker compose --profile infra --profile platform up -d config-server eureka-server
	@echo ""
